...
import type { ExecutionStatus } from "../src/lib.js";
...
describe("extractOutput", () => {
	...
	it("throws when specified block has no output", () => {
		...
		assert.throws(
			() => extractOutput({ execution, outputBlock: "empty-block", outputFile: tmpFile }),
			/No output found in workflow block "empty-block"/,
		);
	});
	...
	it("throws when specified block does not exist", () => {
		...
		assert.throws(
			() => extractOutput({ execution, outputBlock: "missing-block", outputFile: tmpFile }),
			/No output found in workflow block "missing-block"/,
		);
	});
...
describe("triggerWorkflow", () => {
	it("returns execution ID on success", async () => {
		...
		globalThis.fetch = mock.fn(async () =>
			Response.json({ success: true, executionId: "exec-123" }),
		) as typeof fetch;
	...
	it("throws on invalid response body", async () => {
		const originalFetch = globalThis.fetch;
		globalThis.fetch = mock.fn(async () => Response.json({ success: false })) as typeof fetch;
